...
from openapi_server.models.operator_values import OperatorValues
try:
...
class SearchFilter(BaseModel):
    """
    Set the value for a field to filter (scope) the results before search is executed.
    """ # noqa: E501
    var_field: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The name of the search field to filter on. Fields are:  * idServiceAtLocation  * idOrganization  * idService  * idLocation  * tagsService  * dataOwner  * dataOwnerDisplayName  * targetTerm  * address/city  * address/county  * address/stateProvince  * address/postalCode  * address/country  * taxonomy/taxonomyTerm  * taxonomy/taxonomyCode  * taxonomy/target  * serviceAreas/postalCode  * serviceAreas/city  * serviceAreas/county  * serviceAreas/state  * serviceAreas/country", alias="field")
    value: Optional[Any] = Field(description="The value to filter the field on.")
    operators: Optional[OperatorValues] = None
    __properties: ClassVar[List[str]] = ["field", "value", "operators"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "field": obj.get("field"),
            "value": obj.get("value"),
            "operators": OperatorValues.from_dict(obj.get("operators")) if obj.get("operators") is not None else None
        })
        return _obj
